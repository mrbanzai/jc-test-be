<?php
class Api_SchedulesController extends Skookum_Api_Server
{
    ...
    public function init()
    {
        ...
        // load the model(s)
        $this->_schedule = new Api_Schedule($this->_global['websiteUrl']);
    }

    /**
     * Retrieval of all schedules with available jobs. Includes counts for
     * each schedule.
     *
     * @access  public
     * @return  void
    ...
    public function indexAction()
    {
		...
		if (!$this->hasValidParameters()) {
            ...
            if (!in_array($orderBy, array('schedule', 'count'))) {
                $orderBy = 'schedule';
            }
            ...
            $response['data'] = $this->_schedule->getAllByUserId(
                $this->user,
                $page,
                $perPage,
                $orderBy,
		...
		// return the data
		return $this->sendFile($response, 'schedules.json');
    }

    /**
     * Retrieval of the total number of schedules with available jobs.
     *
     * @access  public
     * @return  void
     */
    public function countAction()
    {
		...
		if (!$this->hasValidParameters()) {
            ...
            // retrieve with sorting
            $response['data'] = $this->_schedule->getCountByUserId($this->user);
        }

		// return the data
		return $this->sendFile($response, 'schedules_count.json');
    }
